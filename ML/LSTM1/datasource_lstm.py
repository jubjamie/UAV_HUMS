...
def get_data(asdict=False, newdata=False):
    ...
    if newdata:
        ...
        # Save new data
        np.save(mydir[0] + 'data_lstm.npy', data)
        np.save(mydir[0] + 'labels_lstm.npy', labels)
    else:
        print('Loading data from file...')
        data = np.load(mydir[0] + 'data_lstm.npy')
        labels = np.load(mydir[0] + 'labels_lstm.npy')
